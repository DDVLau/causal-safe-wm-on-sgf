...
class ReplayBufferSafeRL:
    """Replay buffer for storing transitions with costs."""
    ...
    def __init__(self, observation_space, action_space, capacity, start_o, device):
        ...
        self.next_costs = torch.zeros(capacity, dtype=torch.float32, device=device)
    ...
    def step(self, a, next_o, next_r, next_c, next_term, next_trunc, cont_o):
        ...
        if next_term or next_trunc:
            ...
            self.episode_reward = 0.0
            self.episode_costs.append(self.episode_cost)
            self.episode_cost = 0.0
    ...
    def get_stats(self):
        ...
        episode_costs = np.array(self.episode_costs)
        stats = {
            ...
            "buffer_max_episode_cost": np.max(episode_costs),
            "buffer_total_cost": np.sum(episode_costs),
        }
        return stats
